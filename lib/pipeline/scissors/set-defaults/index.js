...
var setTypes = function setTypes(opts) {
  ...
  opts.types = [new _algorithms.Enhanced('rocks')];
};
...
var convertStrings = function convertStrings(types, type, index) {
  ...
  switch (type[0]) {
    case '+':
      _type = type.replace(/^\+/, '');
      types[index] = new _algorithms.Logic(_type);
      break;
    case '*':
      _type = type.replace(/^\*/, '');
      types[index] = new _algorithms.Enhanced(_type);
      break;
    default:
      types[index] = new _algorithms.Scope(type);
  }
};
...
var tidy = function tidy(type) {
  ...
  _algorithms.Scope.params.map(inherit(type, new _algorithms.Scope(pathname)));
};
